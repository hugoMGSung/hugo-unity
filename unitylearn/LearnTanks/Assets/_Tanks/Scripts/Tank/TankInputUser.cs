...
namespace Tanks.Complete
{
    ...
    public class TankInputUser : MonoBehaviour
    {
        ...
        public void ActivateScheme(string name)
        {
            ...
            var scheme = m_LocalActionAsset.FindControlScheme(name);

            if (scheme.HasValue)
            {
                m_InputUser.ActivateControlScheme(scheme.Value);
            }
            else
            {
                Debug.LogWarning($"Control Scheme '{name}' 없음. Scheme 없이 Actions만 연결합니다.");
            }
